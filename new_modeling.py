        ...
        **kwargs,
        ...
        if self.is_add_MoAI_mixer:
            if mode == 'moai_step1.yaml':
                ...
                gate_weights.update(soft_img_weight, soft_lang_weight)
                # MoAI-IMG
                mix_img =\
                ...
                if sum(im_mask[0])!=0 or sum(lang_mask[0])!=0:
                    ...
                    for beam_index in range(hidden_states.shape[0]):
                        ...
                        gate_weights['img_aux'] = soft_img_weight[:,0].mean().item()
                        gate_weights['img_lang'] = soft_img_weight[:,1].mean().item() 
                        gate_weights['img_self'] = soft_img_weight[:,2].mean().item()
                        gate_weights['lang_aux'] = soft_lang_weight[:,0].mean().item()
                        gate_weights['lang_img'] = soft_lang_weight[:,1].mean().item()
                        gate_weights['lang_self'] = soft_lang_weight[:,2].mean().item()
